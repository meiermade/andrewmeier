    ...
    testList "View" [
        ...
        testList "Document" [
            test "includes consent banner and delayed analytics loading" {
                ...
                Expect.stringContains html "fetch('/privacy/consent'" "Expected consent to be persisted by the server"
                Expect.stringContains html "localStorage.removeItem('analytics-consent')" "Expected legacy local consent to be removed after migration"
                Expect.isFalse (html.Contains "localStorage.setItem('analytics-consent',value)") "Expected no new local-only consent choices"
